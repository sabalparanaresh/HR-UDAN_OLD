...
function replaceInvokeWithFetchApi(filePath) {
  ...
  cmds.forEach(cmd => {
    ...
    const regex1 = new RegExp("await\\\\s+invoke(?:Command)?(?:<[^>]+>)?\\\\(\\\\s*['\\\"]" + cmd + "['\\\"]\\\\s*,\\\\s*(\\\\{[\\\\s\\\\S]*?\\\\})\\\\s*\\\\)", 'g');
    content = content.replace(regex1, "await fetchApi('/api/master-data/cmd/" + camelCmd + "', { method: 'POST', body: JSON.stringify($1) })");
    
    const regex2 = new RegExp("invoke(?:Command)?(?:<[^>]+>)?\\\\(\\\\s*['\\\"]" + cmd + "['\\\"]\\\\s*,\\\\s*(\\\\{[\\\\s\\\\S]*?\\\\})\\\\s*\\\\)", 'g');
    content = content.replace(regex2, "fetchApi('/api/master-data/cmd/" + camelCmd + "', { method: 'POST', body: JSON.stringify($1) })");

    const regex3 = new RegExp("await\\\\s+invoke(?:Command)?(?:<[^>]+>)?\\\\(\\\\s*['\\\"]" + cmd + "['\\\"]\\\\s*\\\\)", 'g');
    content = content.replace(regex3, "await fetchApi('/api/master-data/cmd/" + camelCmd + "', { method: 'POST' })");
  });
