...
const AttendanceEntry: React.FC<{ currentUser: UserType | null }> = ({ currentUser }) => {
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      const data = await fetchApi<AttendanceLog[]>(`/api/attendance/logs?${queryParams.toString()}`);
  ...
  const handleFetchRecords = async () => {
    ...
    try {
      ...
      const biometricLogs = await fetchApi<any[]>(`/api/attendance/biometric/sync?module_type=${currentMode}`, {
        method: 'GET'
      });
      
      // Save to backend
      const result = await fetchApi<any>('/api/attendance/biometric/save', { 
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'x-module-type': currentMode },
        body: JSON.stringify({ logs: biometricLogs })
      });
  ...
  const handleManualSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await fetchApi('/api/attendance/manual-punch', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          empId: parseInt(manualData.emp_id),
          date: manualData.date,
          punchIn: punchInFull,
          punchOut: punchOutFull,
          shiftId: null
        })
      });
    ...
    try {
      await fetchApi('/api/attendance/manual-punch', {
        ...
        body: JSON.stringify({
          empId: log.emp_id,
          date: log.date,
          punchIn: edit.punch_in,
          punchOut: edit.punch_out,
...
const GhostPunchesModal: React.FC<GhostPunchesModalProps> = ({ 
  ...
  const handleGenerate = async () => {
    ...
    try {
      const stats = await fetchApi<any>('/api/attendance/resolve-anomalies', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
...
const BulkEntryModal: React.FC<BulkEntryModalProps> = ({ 
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const res = await fetchApi<any>('/api/attendance/bulk-generate-v2', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'x-module-type': currentMode
